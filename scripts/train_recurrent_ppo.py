...
def train(env_id="Ant-v5", direction=None, terrain_curriculum=False, n_envs=8, total_timesteps=10000000):
    """
    Train a RecurrentPPO agent.
...
def train_all_directions(env_id="Walker2d-v5", terrain_curriculum=False, n_envs=8, total_timesteps=10000000):
    """
    ...
    for direction in directions:
        print(f"\n{'='*60}")
        print(f"Training {direction.upper()} policy with RecurrentPPO")
        print(f"{'='*60}\n")
        train(
            env_id=env_id,
            direction=direction,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train RecurrentPPO agent for locomotion")
